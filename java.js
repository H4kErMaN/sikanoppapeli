(() => {
  ...
  function renderRules() {
    const dc = parseInt(diceCountSel.value, 10) || 2;
    rulesList.innerHTML = "";
    const ul = [
      "Heitä noppaa ja kerää kierrospisteitä.",
      dc === 1
        ? "Jos noppa on 1 → kierros nollautuu ja vuoro vaihtuu."
        : "Jos jompikumpi noppa on 1 → kierros nollautuu ja vuoro vaihtuu. Jos molemmat ovat 1 → myös kokonaisscore nollautuu.",
      "“Pankita” lisää kierrospisteet kokonaisscoreen ja vaihtaa vuoron.",
      "Ensimmäinen, joka saavuttaa tavoitepisteet, voittaa."
    ];
    ul.forEach((t) => {
  ...
  diceCountSel.addEventListener("change", renderRules);
  ...
  function startGame() {
    ...
    const diceCount = clamp(parseInt(diceCountSel.value, 10) || 2, 1, 2);
    ...
    diceOut.textContent = String(diceCount);
    renderPlayers();
    setActive(0);
  ...
  function roll() {
    ...
    if (S.diceCount === 2) {
      ...
      if (d1 === 1 && d2 === 1) {
        S.scores[S.active] = 0; updateScore(S.active);
        S.current = 0; updateCurrent(S.active);
  ...
  function updateScore(i) { $(`score_${i}`).textContent = String(S.scores[i]); }
  function updateCurrent(i) { $(`current_${i}`).textContent = String(S.current); }
  ...
  function setDiceVisible(on, d1 = null, d2 = null) {
    ...
    if (S && S.diceCount === 2) {
      ...
      die2El.style.visibility = "hidden";
      die2El.textContent = "—";
    }
  }

  function setStatus(msg) { statusEl.textContent = msg; }
  function enableActions(on) { rollBtn.disabled = !on; holdBtn.disabled = !on; }
